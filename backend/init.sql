-- 1. EXTENSIONS (Crucial for Auth and UUIDs)
...
-- 4. SEEDS
-- Important: Replace credentials manually here if not using dynamic project creation
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM inercia_sys.projects WHERE slug = 'default') THEN
        ...
        VALUES (
            'Inércia Default', 
            'default', 
            'postgres://postgres:qw2silx8ttrevootrxenx3qgjqd2qnfp@postgres:5432/inercia_base', 
            'http://localhost:3000', 
            3000
        );
    END IF;
